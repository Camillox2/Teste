...
export default function ProductInspector() {
  ...
  return <section className="product-inspector yr3-section" id="conheca-de-perto"><div className="yr3-width">
    <div className="inspector-heading"><div><h2>Conheça de perto.</h2><p>Toque nos pontos da imagem.</p></div><label>Explorar equipamento<select value={productId} onChange={event => { setProductId(Number(event.target.value)); setPoint(0) }}>{products.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div>
