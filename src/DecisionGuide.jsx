...
export default function DecisionGuide({ onContinue }) {
  const [place, setPlace] = useState('Em casa')
  ...
  const context = { interest: result.interest, period, message: `Local de uso: ${place}. Tempo de uso: ${period}. Gostaria de comparar as opções de compra e locação.` }
    ...
    <div className="container decision-studio__grid">
      ...
      <div className="decision-studio__panel">
        <fieldset><legend>01 / Onde será o uso?</legend><div className="choice-row">
          {['Em casa', 'Clínica ou instituição'].map(value => <label key={value} className={place === value ? 'choice is-selected' : 'choice'}><input type="radio" name="local-de-uso" value={value} checked={place === value} onChange={() => setPlace(value)} /><span>{value}</span></label>)}
        </div></fieldset>
        <fieldset><legend>02 / Por quanto tempo?</legend><div className="choice-row choice-row--three">
          {['Temporário', 'Contínuo', 'Ainda não sei'].map(value => <label key={value} className={period === value ? 'choice is-selected' : 'choice'}><input type="radio" name="periodo-de-uso" value={value} checked={period === value} onChange={() => setPeriod(value)} /><span>{value}</span></label>)}
        </div></fieldset>
